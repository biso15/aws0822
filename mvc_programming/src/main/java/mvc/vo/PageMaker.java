...
public class PageMaker {
	...
	private void calcData() {
		...
		endPage = (int)(Math.ceil(cri.getPage() / (double)displayPageNum) * displayPageNum);  // 모두 올림처리 하는 메소드 ceil()
		// Math.ceil(cri.getPage() / (double)displayPageNum -> 현재 페이지를 한번에 보여주는 페이지 수로 나눠서 몇번째 페이지 네비게이션인지 확인한다.
		// Math.ceil(cri.getPage() / (double)displayPageNum) * displayPageNum -> 페이지 네비게이션에 한번에 보여주는 페이지 수를 곱해서 마지막 페이지를 구한다.
		// (int)(Math.ceil(cri.getPage() / (double)displayPageNum) * displayPageNum) -> int형으로 형변환
		...
		// totalCount / (double)cri.getPerPageNum() -> 총 게시물수를 한 페이지에 보이는 게시물 수로 나눠서 전체 페이지수를 구한다.
		// Math.ceil(totalCount / (double)cri.getPerPageNum()) -> 남는 게시물 갯수(나머지)가 있을수 있으니 소수점 올림 한다.
		// (int)(Math.ceil(totalCount / (double)cri.getPerPageNum()))) -> int형으로 형변환
